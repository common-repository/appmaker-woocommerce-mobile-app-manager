...
class APPMAKER_WC_REST_Orders_Controller extends APPMAKER_WC_REST_Posts_Abstract_Controller {
	...
	public function prepare_item_for_response( $post, $request ) {
		...
		if ( ! empty( $gateway ) ) {
			...
			$data['show_account_details'] = false;
			if(!empty($data['account_details'])) {
				 $data['show_account_details'] = true;
				...
				$data['account_details_array_title'] = __( 'Account details:', 'woocommerce' );
				if( is_array( $data['account_details'] ) ) {
					foreach( $data['account_details'] as $id => $details ) {					
						if( isset( $details['account_name']) ) {
							$data['account_details_array'][] = array( 'label' =>__('Account name', 'woocommerce'), 'value' =>  $details['account_name'] );						
						}
						if( isset( $details['account_number']) ) {
							$data['account_details_array'][] = array( 'label' =>__('Account number', 'woocommerce'), 'value' =>  $details['account_number'] );						
						}
						if( isset( $details['sort_code']) ) {
							$data['account_details_array'][] = array( 'label' =>__('Sort code', 'woocommerce'), 'value' =>  $details['sort_code'] );						
						}
						if( isset( $details['bank_name']) ) {
							$data['account_details_array'][] = array( 'label' =>__('Bank', 'woocommerce'), 'value' =>  $details['bank_name'] );						
						}
						if( isset( $details['iban']) ) {
							$data['account_details_array'][] = array( 'label' =>__('IBAN', 'woocommerce'), 'value' =>  $details['iban'] );						
						}
						if( isset( $details['bic']) ) {
							$data['account_details_array'][] = array( 'label' =>__('BIC / Swift', 'woocommerce'), 'value' =>  $details['bic'] );						
						}
					}
				}
			}
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			$price = ((get_option( 'woocommerce_prices_include_tax', 'no' )=='no') && (get_option( 'woocommerce_tax_display_shop','inc' )=='incl' ))?$order->get_item_total( $item, true):$order->get_item_total( $item, false, false );
