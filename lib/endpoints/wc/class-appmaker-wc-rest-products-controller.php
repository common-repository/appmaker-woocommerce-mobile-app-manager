...
class APPMAKER_WC_REST_Products_Controller extends APPMAKER_WC_REST_Posts_Abstract_Controller {
    ...
    public function register_routes() {
        ...
        register_rest_route( $this->namespace, '/' . $this->rest_base .'/comments/report', array(
            array(
                'methods'             => WP_REST_Server::CREATABLE,
                'callback'            => array( $this, 'report_comment' ),
                'permission_callback' => array( $this, 'api_permissions_check' ),
                'args'                => $this->get_collection_params(),
            ),
            'schema' => array( $this, 'get_public_item_schema' ),
        ...
        //product detail in blocks
        register_rest_route( 'appmaker-wc/v2', '/' . $this->rest_base .'/(?P<id>[\d]+)', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
                'callback'            => array( $this, 'get_item_blocks' ),
                'permission_callback' => array( $this, 'get_item_permissions_check' ),
                'args'                => array(
                    'context' => $this->get_context_param( array( 'default' => 'view' ) ),
                ),
        ...
        register_rest_route( 'appmaker-wc/v2', '/' . $this->rest_base .'/related/(?P<id>[\d]+)', array(
            array(
                'methods'             => WP_REST_Server::READABLE,
    ...
    public function get_filters($request) {
        ...
        if ( isset ( $_GET['delete_cache'] )  ) {
            ...
            delete_transient( 'appmaker_wc_products_filters_response_' . $hash );
        ...
        if ( ! ( $min_price == $max_price || $min_price > $max_price || APPMAKER_WC::$api->get_settings( 'hide_price_from_filter', false )) ) {
            $return['items']['price_filter'] = array(
                'type'  => 'multi_slider',
                'id'    => 'price_filter',
                'label' => __( 'Price', 'woocommerce' ),
                'min'   => ( $min_price < 0 ) ? 0 : (double) $min_price,
        ...
        foreach ( $attrs_raw as $filter ) {
            ...
            if(!empty($cat_name ) && !is_wp_error($cat_name )){
                ...
                if( $all_categories ) {
                    $return['items'][ 'category' ] = array(
                        'id'    => 'category',
                        'type'  => 'checkbox',
                        'label' => __( 'Categories', 'woocommerce' ),
                    );
                    foreach( $all_categories as $category ) {
                        $return['items'][ 'category' ]['values'][] = array(
                            'label' => $this->decode_html( $category->name ),
    ...
    public function prepare_item_for_response( $post, $request ) {
        ...
        if ( $product->is_type( 'variable' ) && $product->has_child() && $expanded ) {
            ...
            if ( isset( $this->product_image_ids[ $post->ID ] ) ) {
                $this->product_image_ids[ $post->ID ] = array_values( array_unique( $this->product_image_ids[ $post->ID ] ) );
                $data['images_meta']               = $this->get_images_meta( $this->product_image_ids[ $post->ID ] );
            }
        }
       ...
       // $data    = $this->add_additional_fields_to_object( $data, $request ); // removed due to product list slow response 
    ...
    public function get_product_in_webview( $product ) {
        ...
        $data = array();
        $products_in_webview         = apply_filters( 'appmaker_wc_products_in_webview', array( 'bundle','yith_bundle','lottery','grouped','gift-card','composite','woosb','pw-gift-card','woosg' ) );
        ...
        if(! empty($category_in_webview) ) {
            foreach($product_category as $term){
                $category = $term->term_id;
                if(in_array($category, $category_in_webview ) ) {
        ...
        if( in_array($product->get_type() , $products_in_webview ) || in_array($product_id, $products_in_webview ) || $enable_all_products_webview  ) { 
           // $title = $this->decode_html( $product->get_title() );
            $data['product_in_webview'] = true;
            $product_url =  add_query_arg( array( 'from_app' => true , 'key' => true ), $this->ensure_absolute_link( $product->get_permalink() ) );                      
            $data['product_in_webview_action'] = array(                
    ...
    public function get_thumbnail( $product ) {
        ...
        if ( empty( $image ) ) {
            ...
            if( !empty($image[1]) && !empty($image[2])  ) {               
                ...
                $image['size'] = APPMAKER_WC_Helper::get_image_dimensions($thumb_post ,$size ,true );
            }
    ...
    protected function product_widgets( $product_obj, $data ) {
        ...
        foreach ( $tabs as $key => $tab ) {
            ...
            if ( $key == 'reviews' && ( $tab_type == 'DEFAULT' ) ) {
              ...
              //  if ( ! empty( $reviews ) && ! empty( $reviews->data ) ) {
                    $return[ $key ] = array(
                        ...
                        'allow_review_report' => APPMAKER_WC::$api->get_settings('allow_visitors_to_report_comments', true ),
            ...
            elseif ( $key == 'additional_information'|| $key=='specifications' ||  $key=='specification'&& $tab_type == 'DEFAULT' ) {
                ...
                $related_products = apply_filters( 'appmaker_wc_related_products' , $related_products , $product );
    ...
    public function get_content( $request ) {
        ...
        global $product,$post,$wp_query,$withcomments;     
        $withcomments = 1;
        ...
        $content = apply_filters('appmaker_product_widget_webview_content', $content,  $tab_id );
    ...
    public function get_reviews( $request, $limit = 0 ) {
        ...
        foreach ( $reviews as $review ) {
            ...
            $images = apply_filters( 'appmaker_review_images' , array() , $review, $product );
   ...
   /*     $images            = $this->get_images( $variation, true, APPMAKER_WC_Helper::get_id( $product ) );
    ...
    public function get_items( $request ) {
       ...
       if( isset( $args['orderby'] ) && !empty( $args['orderby'] ) && empty( $request['orderby'] ) ) {
            $request['orderby'] =  $args['orderby'];
       }
       if( isset( $args['order'] ) && !empty( $args['order'] ) && empty( $request['order'] ) ) {
            $request['order'] =  $args['order'];
       }
        $sort_args    = WC()->query->get_catalog_ordering_args( $request['orderby'], $request['order'] );
        if( isset( $request['prebuild']) && isset( $args['meta_key'] ) && !empty( $args['meta_key'] ) ) {
            $sort_args['meta_key'] = $args['meta_key'];
        }
        ...
        //unset fields if value is id=>parent
        if( isset($query_args['fields']) && 'id=>parent' == $query_args['fields'] ){
    ...
    public function get_item( $request ) {
        ...
        if ( isset ( $_GET['delete_cache'] )  ) {
            $hash = $this->get_request_hash( $request );
            delete_transient( 'appmaker_wc_products_filters_response_' . $hash );
        }else if ( APPMAKER_WC::$api->get_settings( 'cache_enabled', false ) ) {
            $hash = $this->get_request_hash( $request );
    ...
    public function get_products_list( $request ){
        ...
        switch($key){
            ...
            case 'top_rated':
                ...
                break;
            case 'category' : 
                $products = APPMAKER_WC_Helper::get_products_by_tax( 'product_cat', $request['id'], $language ); 
                break;
            case 'tag' : 
                $products = APPMAKER_WC_Helper::get_products_by_tax( 'product_tag', $request['id'], $language ); 
                break;
    ...
    public function report_comment( $request ) {
        ...
        if( isset( $request['id']) && ! empty( $request['id']) ) {
            ...
            $subject = sprintf( 'A comment by %s has been flagged by a user', esc_html( $comment->comment_author ) );
            $headers = sprintf( 'From: %s <%s>', esc_html( get_bloginfo( 'site' ) ), get_option( 'admin_email' ) ) . "\r\n\r\n";
            $message =  'A user of your App has flagged a comment.' . "\r\n";
            ...
            $return['status'] = true;
            $return['message'] = __( 'Thank you for your feedback. We will look into it.', 'appmaker-woocommerce-mobile-app-manager' );
        }
    ...
    public function get_item_blocks( $request ) {
        $extra_blocks = array();
        // $this->send_cache_header();
        // if ( isset ( $_GET['delete_cache'] )  ) {
        //     $hash = $this->get_request_hash( $request );
        ...
        if( !is_wp_error( $response ) && isset( $response['product_widgets'] ) && ! empty( $response['product_widgets'] ) ) {
            $data =  $response['product_widgets'];
            unset(  $response['product_widgets'] );
            $index = 0;
            foreach( $data as $product_widget ) {//print_r($product_widget);exit;            
                $type = $product_widget['type'];               
                switch( $type ) {
                    case 'text' : $content = $product_widget['content'] ? $product_widget['content'] : '';
                        $extra_blocks[] = array( 'name' => 'appmaker/expandable-text-block',
                            'clientId' => 'widget-'.$index,
                            'attributes' => array( 'blockTitle' => $product_widget['title'] , 
                                'content'           => $content,
                                'accessButton'      => $product_widget['expandable'] ? 'View' : false,
                                'expanded'          => $product_widget['expanded'] ,
                                'expandable'        => $product_widget['expandable'] ,
                            ),
                            'innerBlocks' => array( 'name' => 'appmaker/text' , 'attributes' => array( 'content' => $content ) ),
                        ); 
                        $index++;
                        break;
                    case 'menu' :  $attribute_type = '';
                        if( isset( $product_widget['action'] ) && isset( $product_widget['action']['type'] ) ) {
                            $attribute_type = $product_widget['action']['type'] === 'NO_ACTION' ? 'title' : '';
                        }
                        $extra_blocks[] = array( 'name' => 'appmaker/actionbar',
                            'clientId' => 'widget-'.$index,
                            'attributes' => array( 'type' =>  $attribute_type , 
                                'title'  => $product_widget['title'],
                                'appmakerAction' => $product_widget['action'],
                                
                            ),                           
                        );
                        $index++;
                        break;
                    case 'reviews' : $reviews = $product_widget['items']->data;
                        $extra_blocks[] = array( 'name' => 'appmaker/block-card',
                            'clientId' => 'widget-'.$index,
                            'attributes' => array( 'title' => $product_widget['title'] , 
                                'productId'           => $product_id,
                                'accessButton'      => $product_widget['expandable'] ? 'View' : false,
                                'expanded'          => $product_widget['expanded'] ,
                                'expandable'        => $product_widget['expandable'] ,
                             ),
                            'innerBlocks' => array( 'blockTitle' => $product_widget['title'] , 
                                'name' => 'appmaker/product-review' , 
                                'attributes' => array( 'productId' =>  $product_id,
                                    'productReview' => $reviews,
                                    'canReview'     =>  $product_widget['allow_product_review'],
                                    'view_more_reviews' =>  $product_widget['view_more_reviews'],
                                    'allow_review_report' =>$product_widget['allow_review_report'],
                                    'appmakerAction'  =>  array( 'action' =>  'REPORT_COMMENT', 'params' => array() ),
                                ),
                            ),
                        ); 
                        $index++;
                        break;
                    case 'key_value' : 
                        $contents = $product_widget['items'];
                        $innerBlocks = array();
                        if( ! empty( $contents ) ) {
                            foreach( $contents as $content ) {
                                $innerBlocks[] = array( 'name' => 'appmaker/table-cell', 'attributes' => array( 'title' => $content['label'] , 'value' => $content['value'] ) );
                            }
                        }                        
                        $extra_blocks[] = array( 'name' => 'appmaker/block-card',
                            'clientId' => 'widget-'.$index,
                        ...
                        break;
                    case 'vendor_card':
                        $extra_blocks[] = array( 'name' => 'appmaker/vendor-card',
                            'clientId' => 'widget-'.$index,
                            'attributes' => array( 'title' => $product_widget['title'] ,                            
                                'featureImg'      => $product_widget['image'],
                                'vendorName'      =>  $product_widget['title'],
                                'appmakerAction'  => $product_widget['action'],
                            ),                        
                        ); 
                        $index++;
                        break;
                    case 'product_scroller':  $extra_blocks[] = array( 'name' => 'woocommerce_related_products', 
                            'attributes' => array( 'title' => $product_widget['title'] , 'value' => $product_id 
                            ), 
                        );
                        $index++;
                        break;
                    default:
                        $content = isset( $product_widget['content'] ) && !empty($product_widget['content'] ) ? $product_widget['content'] : '';
                            $extra_blocks[] = array( 'name' => 'appmaker/expandable-text-block',
                                'clientId' => 'widget-'.$index,
                                'attributes' => array( 'blockTitle' => $product_widget['title'] , 
                                    'content'           => $content,
            ...
            $response['extra_blocks'] = $extra_blocks;
        }
