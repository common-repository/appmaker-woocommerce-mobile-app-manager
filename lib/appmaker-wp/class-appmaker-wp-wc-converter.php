...
class APPMAKER_WP_WC_Converter {
	...
	public static function convert_inAppPage_data( $data_obj, $key ) {
		...
		if ( class_exists( 'APPMAKER_WC' ) && ! isset( APPMAKER_WC::$api->APPMAKER_WC_REST_Posts_Controller ) ) {
			$post_controller = new APPMAKER_WC_REST_Posts_Controller();
		} elseif ( class_exists( 'APPMAKER_WC' ) ) {
			$post_controller = APPMAKER_WC::$api->APPMAKER_WC_REST_Posts_Controller;
		}
		...
		foreach ( $data_obj->widgets as $value_obj ) {
			...
			if ( 'pre_build_product_scroller' === $widget['type'] ||  'pre_build_product_grid' === $widget['type'] ) {
			...
			if ( 'pre_build_post_scroller' === $widget['type'] || 'pre_build_post_grid' === $widget['type']  ) {
					$value->data =  APPMAKER_WC_Helper::get_posts_by_tax( 'category', $value->data[0]->data->action_value->value, $language );
				if ( empty( $value->data ) ) {
					$skip = true;
				} else {
					if ( ! $skip_shuffle ) {
			...
			if ( is_array( $value->data ) || is_object( $value->data ) ) {
				foreach ( $value->data as $value_data ) {
					$data = false;
					if ( 'product_scroller' === $widget['type'] || 'product_scroller_grid' === $widget['type'] ) {
					...
					} elseif ( 'pre_build_product_scroller' === $widget['type'] || 'pre_build_product_grid' === $widget['type'] ) {
						$product = APPMAKER_WC_Helper::get_product( $value_data );
					...
					} elseif ( 'pre_build_post_scroller' === $widget['type'] || 'pre_build_post_grid' === $widget['type'] ) {	
						if ( ! isset( $value_data->data->action_value->value ) && ( is_string( $value_data ) || is_numeric( $value_data ) ) ) {
							$request['id'] = $value_data ;
						} else {
							$request['id'] = $value_data->data->action_value->value ;

						}
						//$request['id'] = $value_data;					
						$request['context'] = 'view' ;
						$post_data = $post_controller->get_items_post_widget($request);	
						$data      = $post_data->data;
						...
						if ( isset( $value_data->data->action_value->dependent_value->value ) ) {
							...
							$data['action']['params'] = self::get_params( $value_data->data->action->value, $values_array );
						}
					}
					if ( false !== $data ) {
						$widget['data'][] = $data;
			...
			if ( ! empty( $value->data_main ) ) {
				...
				foreach ( $value->data_main as $value_key => $value_data ) {
					$values = self::getValue( $value_data );
					if ( preg_match( '/(.*)_action$/i', $value_key, $matches ) ) {
						...
						if ( is_string( $values ) ) {
							$widget['meta'][ $value_key ]['original_type'] = $values;
						} else {
							$widget['meta'][ $value_key ]['original_type'] = isset( $values->action->value ) ? $values->action->value : $values->action->id;
						}
						$widget['meta'][ $value_key ]['type'] = self::get_type( $values );
						...
						if ( ! isset( $widget['meta'][ $action_key ]['type'] ) ) {
							$widget['meta'][ $action_key ]['params'] = $values;
						} else {
							$widget['meta'][ $action_key ]['params'] = self::get_params( $widget['meta'][ $action_key ]['original_type'], $values );
						}
					} elseif ( preg_match( '/(.*)_analytics$/i', $value_key, $matches ) ) {
						$action_key                                 = $matches[1] . '_action';
						$widget['meta'][ $action_key ]['analytics'] = $values;
					} else {
						$widget['meta'][ $value_key ] = $values;
					}
				}
			}
			...
			if ( 'product_scroller_grid' === $widget['type'] ) {
				$widget['type'] = 'pre_build_product_grid';
			}
