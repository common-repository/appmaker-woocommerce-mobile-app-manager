...
class APPMAKER_WP_WC_REST_BACKEND_MEDIA_Controller extends APPMAKER_WP_WC_REST_Controller {
	...
	public function search_media( $request )
    {
        ...
        foreach ($q->get_posts() as $attachment){
            ...
            $attachments[] = array('id' => $response, 'label' => $attachment->post_title) ;
        }
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if( empty($meta) || (is_array( $meta ) && empty( $meta['width'] ) || empty( $meta['height'] ) ) ) {
        ...
        $url = apply_filters('appmaker_wc_in_app_page_media', wp_get_attachment_url($item->ID));
		$response = array( 'title'=> $item->post_title, 'id' => $item->ID, 'url' => $url, 'meta' => array('width' => $meta['width'],'height' => $meta['height']) );
        return $response;
	}
