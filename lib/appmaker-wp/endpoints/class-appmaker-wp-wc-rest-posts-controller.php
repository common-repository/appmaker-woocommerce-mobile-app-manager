...
class APPMAKER_WP_WC_REST_Posts_Controller extends APPMAKER_WP_WC_REST_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			...
			array(
				array(
					'methods'             => WP_REST_Server::READABLE,
					'callback'            => array( $this, 'get_blog_content' ),
					'permission_callback' => array( $this, 'api_permissions_check' ),
					'args'                => array(
						'context' => $this->get_context_param( array( 'default' => 'view' ) ),
					),
		...
		register_rest_route(
			$this->namespace,
			'/' . 'inAppPages/dynamic/blogs',
			array(
				array(
					'methods'             => WP_REST_Server::READABLE,
					'callback'            => array( $this, 'get_items_dynamic' ),
					'permission_callback' => array( $this, 'api_permissions_check' ),
					'args'                => $this->get_collection_params(),
				),
	...
	// get items for dynamic posts

	public function get_items_dynamic( $request ) {
		$args                        = array();
		$args['author__in']          = $request['author'];
		$args['author__not_in']      = $request['author_exclude'];
		$args['menu_order']          = $request['menu_order'];
		...
		foreach ( $query_result as $post ) {
			...
			$data    = $this->prepare_new_post_response( $post, $request );
			$posts[] = $this->prepare_response_for_collection( $data );
		}
	...
	public function get_items_post_widget( $request ) {
		...
		$data = $this->prepare_new_post_widget_response( $post, $request );
	...
	public function prepare_new_post_widget_response( $post, $request ) {
		...
		if ( method_exists( 'WPBMap', 'addAllMappedShortcodes' ) ) {
			...
			$url                = $base_url . '/?rest_route=/' . $this->namespace .
				'/' . $this->rest_base . '/' . $post->ID . '/content' . '&api_key=' . $api_key;
			$url                = add_query_arg(array('from_app' => true, 'key' => true ), $url);

			$data = array(						
						'title'    => $this->decode_html( get_the_title( $post->ID ) ),
						'image'    => empty($featured_media_url) ? '': $featured_media_url ,
						'content'  => $this->prepare_excerpt_response( $post->post_excerpt, $post ),
	...
	public function get_blog_content( $request ) {
	...
	public function get_content( $post ) {
		...
		if ( ! empty( $featured_media_url ) ) {
			$content = "<img src='$featured_media_url' /> <br/ >\n" . $content;
		}
		if ( empty( $this->head ) ) {
			ob_start();
			wp_head();
		...
		$output = '<!DOCTYPE html>
			...
			return apply_filters( 'appmaker_wc_post_content', $output );
	}
	/**
	...
	public function prepare_item_for_response( $post, $request ) {
		...
		if ( ! empty( $schema['properties']['content'] ) ) {
			...
			$data['content'] = array(
				...
				/** This filter is documented in wp-includes/post-template.php */
				'rendered' => $this->get_content( $post ),
	...
	public function prepare_new_post_response( $post, $request ) {
