...
class APPMAKER_WC_Third_party_support {
	static function init() {
		...
		if ( class_exists( 'WPCleverWoosb' ) && ! class_exists('APPMAKER_WC_WPCLEVER') ) {
			require 'misc/class-appmaker-wc-wpclever.php';
		}

		if ( class_exists( 'OTP\MoOTP' ) && ! class_exists('APPMAKER_WC_MINIORANGE') ) {
			require 'misc/class-appmaker-wc-miniorange-otp.php';
		}
        //https://wordpress.org/plugins/wordpress-seo/
		if ( function_exists( 'wpseo_auto_load' ) && ! class_exists('APPMAKER_WC_WPSEO') ) {
			require 'misc/class-appmaker-wc-wp-seo.php';
		}

		 //https://wordpress.org/plugins/yet-another-related-posts-plugin/
		 if ( class_exists( 'YARPP' ) && ! class_exists('APPMAKER_WC_YARPP') ) {
			require 'misc/class-appmaker-wc-yarpp.php';
		}
		
		if ( class_exists( 'ElasticPress\Elasticsearch' ) && ! class_exists('APPMAKER_WC_ElasticPress') ) {
			require 'misc/class-appmaker-wc-elasticpress.php';
		}
		//https://wordpress.org/plugins/product-input-fields-for-woocommerce/
		if ( class_exists( 'Alg_WC_PIF' ) && ! class_exists('APPMAKER_WC_PRODUCT_INPUT_FIELDS') ) {
			require 'misc/class-appmaker-wc-product-input-fields.php';
		}

		// https://wordpress.org/plugins/woo-discount-rules/
		if ( class_exists( 'Wdr\App\Controllers\ManageDiscount' ) && ! class_exists('APPMAKER_WC_WDR') ) {
			require 'misc/class-appmaker-wc-woo-discount-rules.php';
		}

		//https://villatheme.com/extensions/woocommerce-photo-reviews/
		if ( class_exists( 'VI_WOOCOMMERCE_PHOTO_REVIEWS_Frontend_Shortcode' ) && ! class_exists('APPMAKER_WC_PHOTO_REVIEWS') ) {
			require 'misc/class-appmaker-wc-photo-reviews.php';
		}
