...
class APPMAKER_WC_SUMO_PAYMENT {
    ...
    public function __construct() {
     ...
     public function payment_fixed_depo($return,$params){
		...
		if(method_exists('SUMO_PP_Product_Manager','get_product_props' )){
            ...
            $sumo = new SUMO_PP_Product_Manager;
            $product_props = $sumo->get_props($_POST['product_id'] );
        }            
    ...
    public function get_payment_plans(){
		...
		if(method_exists('SUMO_PP_Product_Manager','get_product_props' )){
            ...
            $sumo = new SUMO_PP_Product_Manager;
            $product_props = $sumo->get_props($product );
        }
    ...
    public function product_fields( $fields, $product ) {
		...
		if(method_exists('SUMO_PP_Product_Manager','get_product_props' )){
            ...
            $sumo = new SUMO_PP_Product_Manager;
			$product_props = $sumo->get_props($product->get_id() );
		}
                    ...
                    foreach ( $payment_plans as $keys ) {
                        foreach ( $keys as $plan_id ) {
							...
							if(class_exists('SUMO_PP_Payment_Plan_Manager')){
                                $sumo_plan_obj                                = new SUMO_PP_Payment_Plan_Manager;
								$payment_plan_data                            = $sumo_plan_obj->get_props( $plan_id );
							}else{
                                $sumo_obj                                     = new SUMO_PP_Plan_Manager;
								$payment_plan_data                            = $sumo_obj->get_props( $plan_id );
							}
