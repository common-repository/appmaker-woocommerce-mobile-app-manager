...
class APPMAKER_WC_General_hooks {
	...
	public function __construct() {
		...
		if ( ( ! empty( $_GET['from_app'] ) && ! empty( $_GET['key'] ) ) || isset( $_COOKIE['from_app_cookie'] ) ) {
			...
			if ( class_exists( 'WC_UepaPay' ) || function_exists('my_faw_plugin_path') || class_exists('Vcb_Mh_Admin') || function_exists('cashapp_fs') || function_exists( 'venmo_fs' ) ) {
		...
		if( ! empty ( $_POST['from_app'] ) ||  ! empty( $_GET['from_app'] )  || isset( $_COOKIE['from_app_cookie'] ) )  {
	...
	public function comment_post_redirect( $location, $comment ) {
        ...
        if( isset($_POST['_wp_http_referer'] ) ){
			...
			$location = $url . '#comment-' . $comment->comment_ID;
