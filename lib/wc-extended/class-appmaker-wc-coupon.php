...
class APPMAKER_WC_Coupon {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_coupon_is_valid', array( $this, 'app_woocommerce_coupon_code_validate' ), 999, 2 );
	...
	public function app_woocommerce_coupon_code_validate( $valid, $coupon ) {
		if ( ! empty( $coupon ) && get_post_meta( $this->get_coupon_id( $coupon ), 'appmaker_wc_app_only_coupon', 'no' ) == 'yes' &&  ! defined( 'APPMAKER_WC_REQUEST' ) && !isset($_COOKIE['from_app_cookie']) ) {
			return false;
		}
	...
	public function app_woocommerce_coupon_error( $err, $err_code, $coupon ) {
		if ( ! empty( $coupon ) && get_post_meta( $this->get_coupon_id( $coupon ), 'appmaker_wc_app_only_coupon', 'no' ) == 'yes' && $err_code == WC_Coupon::E_WC_COUPON_INVALID_FILTERED && ! defined( 'APPMAKER_WC_REQUEST' ) ) {
			return __( 'Sorry, this coupon only valid for app users.', 'appmaker-woocommerce-mobile-app-manager' );
		}
